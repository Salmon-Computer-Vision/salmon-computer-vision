...
gst_writer_str = "appsrc ! video/x-raw,format=BGR ! queue ! videoconvert ! video/x-raw,format=BGRx ! nvvidconv ! video/x-raw(memory:NVMM),format=NV12 ! nvv4l2h264enc vbv-size=200000 bitrate=3000000 insert-vui=1 ! h264parse ! mp4mux ! filesink location="
...
class VideoSaver(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if c % 20 == 0:
                logger.info(f"Saving pid={os.getpid()} file={filename} frame={c}")
    ...
    def stop_video_saving(self, final: bool = True, join_timeout: float = 10.0):
        ...
        if self.save_video and self.video_saver_stop_event is not None:
            ...
            if self.video_saver is not None and self.video_saver.is_alive():
                ...
                self.video_saver.join(timeout=join_timeout)

                if self.video_saver.is_alive():
                    self.log.error(
                        "VideoSaver pid=%s did not exit after %.1fs; terminating",
